...
앱이 실행되는 동안 macOS 파일 변경 이벤트로 Claude Code 기록을 감시합니다. 대시보드 창이나 메뉴 팝오버를 닫아도 갱신되며, 짧은 연속 쓰기는 모아서 처리합니다. 60초 주기 새로고침은 앱 전체에서 보조 수단으로 실행됩니다. 앱 자체를 Quit하면 감시도 종료됩니다.
...
화면에는 마지막 수신 시각을 표시합니다. 같은 값이 다시 전달되어도 값 변경 시각은 갱신하지 않으며, 5분 이상 값이 같거나 수신이 없으면 오래된 값일 수 있다고 표시합니다. 이는 서버의 마지막 측정 시각을 보장하는 정보가 아닙니다. 데이터가 없는 창은 `—`, 초기화 시각이 지난 창은 재수신 대기로 표시합니다.
