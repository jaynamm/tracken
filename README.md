...
Total의 일별 차트는 Codex(파란색)와 Claude(주황색)를 누적 막대로 구분합니다. 토큰·예상 비용 모두 같은 색상과 범례를 사용하며, 날짜 위에 마우스를 올리면 정보창에서 플랫폼별 토큰 사용량·예상 비용과 합계를 함께 확인할 수 있습니다. 개별 플랫폼 차트도 해당 날짜의 토큰과 비용을 함께 표시합니다. 마우스가 차트를 벗어나면 정보창이 사라집니다. 비용을 일부만 계산한 구간은 연하게 표시하고, 해당 날짜의 비용을 전혀 계산할 수 없는 플랫폼은 비용 막대에서 제외합니다. 정보창에서도 부분 추정과 비용 미확정을 구분합니다.
