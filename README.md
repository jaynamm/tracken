...
앱은 메인 창과 메뉴 막대 항목을 함께 제공합니다. 대시보드 상단의 tracken 왼쪽에는 배경 없는 컬러 마스코트를 표시합니다. 메뉴 막대에는 같은 마스코트를 단색으로 표시하며, macOS의 밝은·어두운 메뉴 막대에 맞춰 색상이 바뀝니다. Dock과 Finder에는 어두운 배경의 컬러 마스코트 앱 아이콘을 표시합니다.
...
Total의 일별 차트는 Codex(파란색)와 Claude(주황색)를 누적 막대로 구분합니다. 토큰·예상 비용 모두 같은 색상과 범례를 사용하며, 날짜를 선택하면 플랫폼별 수치도 확인할 수 있습니다. 비용을 일부만 계산한 구간은 연하게 표시하고, 해당 날짜의 비용을 전혀 계산할 수 없는 플랫폼은 비용 막대에서 제외합니다.
