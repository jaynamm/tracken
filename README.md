...
Codex와 Anthropic의 토큰 사용량을 한곳에서 확인하는 macOS 메뉴 막대 앱입니다. Total 대시보드에서 오늘·최근 14일의 통합 토큰과 예상 비용, 플랫폼별 누적 차트를 확인하고 개별 공급자 화면과 메뉴 막대 팝오버로 상세 현황을 볼 수 있습니다.
...
- Total 통합 대시보드와 Codex(ChatGPT)·Claude(Anthropic) 공급자별 화면 전환
- Total에서 오늘·최근 14일 토큰 및 예상 비용 합계, 통합 일별 차트와 플랫폼별 연결 상태·사용 한도 확인
...
앱은 메인 창과 메뉴 막대 항목을 함께 제공합니다. 대시보드 상단의 tracken 왼쪽에는 배경 없는 컬러 마스코트를 표시합니다. 메뉴 막대에는 같은 마스코트를 단색으로 표시하며, macOS의 밝은·어두운 메뉴 막대에 맞춰 색상이 바뀝니다. Dock과 Finder에는 어두운 배경의 컬러 마스코트 앱 전용 아이콘을 표시합니다. 둥근 배경 바깥은 투명하며, Dock 크기에 맞춘 여백을 사용합니다.
...
설정 왼쪽 메뉴는 **언어 / Language**, **Codex**, **Claude**로 나뉩니다. 언어 메뉴에서 **한국어 (Korean)** 또는 **English**를 선택할 수 있으며, 각 공급자 메뉴에서 연결·기록과 해당 공급자의 API 가격표를 관리합니다. 대시보드·메뉴 막대·설정의 문구와 날짜·숫자 표시가 즉시 바뀌며, 선택은 앱을 다시 실행해도 유지됩니다. 최초 실행 시 macOS의 우선 언어가 한국어이면 한국어로, 그 외에는 영어로 시작합니다. 서비스·모델 이름과 외부 시스템에서 전달된 알 수 없는 오류의 원문은 그대로 표시합니다.

Claude의 과거 기록은 그대로 보존됩니다. API 키 삭제는 로컬 사용 기록을 지우거나 조회를 끄지 않습니다. 메뉴 막대의 공급자별 사용량과 합계는 대시보드에서 선택한 기간과 관계없이 최근 14일 기준입니다.

Total은 모든 플랫폼에 동일하게 최근 14일을 적용하며 Claude 개별 화면의 조회 기간과 독립적입니다. 불러온 데이터가 있는 플랫폼만 합산하고 포함된 플랫폼 수를 표시합니다. 데이터가 전혀 없으면 `—`, 불러온 기록에 사용량이 없으면 0을 표시합니다. 새로고침에 실패한 플랫폼의 이전 데이터가 남아 있으면 합계에 포함하고 안내를 표시합니다. 비용을 일부만 계산할 수 있으면 `partial estimate`로 구분하며 플랫폼별 구독 한도는 각각 표시합니다.

Total의 일별 차트는 Codex(파란색)와 Claude(주황색)를 누적 막대로 구분합니다. 토큰·예상 비용 모두 같은 색상과 범례를 사용하며, 날짜 위에 마우스를 올리면 차트 위쪽의 상세 영역에 플랫폼별 토큰 사용량·예상 비용과 합계를 함께 표시합니다. 상세 영역은 항상 자리를 유지하므로 막대를 가리거나 차트 위치를 바꾸지 않습니다. 개별 플랫폼 차트도 해당 날짜의 토큰과 비용을 함께 표시합니다. 마우스가 차트를 벗어나면 기본 날짜 정보로 돌아갑니다. 비용을 일부만 계산한 구간은 연하게 표시하고, 해당 날짜의 비용을 전혀 계산할 수 없는 플랫폼은 비용 막대에서 제외합니다. 상세 영역에서도 부분 추정과 비용 미확정을 구분합니다.

Total의 **일별 사용량 및 예상 비용** 표는 각 날짜의 합계 아래에 Codex·Claude별 토큰 수와 예상 비용을 표시합니다. 플랫폼별 행은 차트와 같은 색을 사용하고, 사용량이 없는 날짜는 0, 비용을 계산할 수 없으면 `—`, 일부만 계산하면 부분 추정으로 구분합니다. 불러온 기록이 없는 플랫폼은 합계와 세부 행에서 제외합니다.
...
Claude의 5시간·7일 구독 한도는 토큰 수로 추정하지 않고 [공식 상태줄 데이터](https://code.claude.com/docs/en/statusline#available-data)에서 받습니다. Claude Code 2.1.251 이상과 Pro/Max 세션의 실제 응답이 필요합니다. 한도값이 아직 없으면 대기 상태를 표시하며, 한도가 초기화될 시각이 지났더라도 0%로 임의 변경하지 않습니다.

현재 tracken의 한도 수신은 **터미널 Claude Code의 상태줄 명령**에 의존합니다. VS Code의 Claude 채팅 패널과 통합 터미널에서 실행하는 `claude`는 다른 실행 방식이므로, 채팅 패널에서 사용했다고 한도 데이터가 전달되는 것은 아닙니다. Pro로 정상 로그인했더라도 상태줄 데이터가 수신되지 않으면 한도가 표시되지 않습니다. 토큰 사용 기록은 로컬 세션 파일에서 별도로 읽기 때문에 한도가 없어도 표시될 수 있습니다.
...
설치 후 VS Code 통합 터미널 또는 일반 터미널에서 `claude`를 실행하고, Pro/Max 계정으로 응답을 받은 뒤 tracken의 새로고침을 누릅니다. `~/Library/Application Support/tracken/Claude/rate-limits.json`이 아직 없다면 유효한 한도 데이터가 저장되지 않은 상태입니다. tracken 새로고침은 이 파일을 다시 읽을 뿐 Claude 서버에 한도를 직접 요청하지 않습니다. 현재 구현에는 VS Code 채팅 패널의 한도를 직접 수집하는 별도 경로가 없습니다.

브리지가 기록한 한도 파일도 tracken에서는 시간별 또는 수동 갱신 때만 읽습니다. 대시보드는 카드당 사용량의 최근 업데이트 날짜와 시간을 한 번만 표시합니다. 사용량이 없는 Claude 상세 카드와 메뉴 막대·설정의 한도 영역에는 마지막 수신 날짜와 시간을 `최근 업데이트`로 표시합니다. 한도 업데이트 시각은 브리지가 데이터를 받은 시각이며 서버의 마지막 측정 시각을 뜻하지 않습니다. 데이터가 없는 창은 `—`, 초기화 시각이 지난 창은 재수신 대기로 표시합니다.
...
로컬 기록은 대화 생성 날짜와 관계없이 조회 기간 안의 토큰 이벤트를 집계합니다. 중복 응답 ID와 누적값이 같은 구형 알림은 한 번만 계산합니다. 공식 표에 긴 컨텍스트 요금이 있는 모델은 272K 초과 입력에 해당 단가를 적용하며, 단가를 모르는 모델이 포함되면 계산 가능한 금액을 `partial estimate`로 구분하고, 계산 가능한 기록이 전혀 없으면 `—`로 표시합니다. 개별 모델의 단가 미지원 상태를 구독에 포함된 비용으로 표시하지 않습니다.

일부 과거 Codex 기록에는 총 토큰 수만 있고 모델명이나 입력·출력 구분이 없습니다. 이 경우 총 토큰은 보존하고 `Details unavailable`로 표시합니다. 세부 정보가 없는 기록의 비용은 미확정으로 유지하며, 함께 있는 계산 가능한 기록의 비용은 일별·기간 합계에서 `partial estimate`로 표시합니다.
...
대시보드 상단에 오늘의 토큰과 예상 비용(USD)을 표시하고, 일별 그래프에서 예상 비용과 토큰을 전환할 수 있습니다. 일별 비용 목록은 모델별 합계보다 먼저 표시됩니다. 사용 기록이 없는 날짜는 $0, 사용량이 있지만 계산 가능한 비용이 전혀 없는 날짜는 `—`로 표시합니다. 일부 기록만 계산할 수 있으면 해당 금액을 `partial estimate`로 표시하며, 날짜 목록·모델별 합계·기간 합계·오늘 요약·메뉴 막대에 같은 구분을 적용합니다. 그래프의 부분 추정치는 연한 막대로 표시합니다. 부분 추정치는 가격을 계산한 로컬 기록만 포함하며 전체 사용량의 비용을 뜻하지 않습니다. 1센트 미만의 양수 비용은 `< $0.01`로 구분합니다.
...
- 설정의 **Codex** 또는 **Claude** 메뉴 안의 **API price tables**에서 모델별 가격, 출처, 마지막 확인 시각을 확인하고 **Update prices now**로 해당 공급자의 가격을 즉시 갱신할 수 있습니다.
...
Dock·Finder용 앱 아이콘은 `Scripts/generate-app-icon.swift`의 벡터 도형으로 관리합니다. 디자인을 수정한 뒤 아래 명령을 실행하면 투명도를 유지한 16–1024px 에셋을 다시 생성합니다. 메뉴 막대 아이콘은 별도 에셋입니다.
